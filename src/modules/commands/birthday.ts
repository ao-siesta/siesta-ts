...
function recentWhoBirthday(sub: SlashCommandSubcommandBuilder) {
  return sub
    .setName('recent')
    .setDescription('最近誰生日？')
}

function listBirthday(sub: SlashCommandSubcommandBuilder) {
  return sub
    .setName('list')
    .setDescription('列出所有生日')
}

function addBirthday(sub: SlashCommandSubcommandBuilder) {
  return sub
    .setName('add')
    .setDescription('加入生日')
...
function delBirthday(sub: SlashCommandSubcommandBuilder) {
  return sub
    .setName('delete')
...
function sendBdCard(sub: SlashCommandSubcommandBuilder) {
  return sub
    .setName('card')
...
const subCommands: ((sub: SlashCommandSubcommandBuilder) => SlashCommandSubcommandBuilder)[] = [
  todayWhoBirthday,
  recentWhoBirthday,
  listBirthday,
  addBirthday,
...
async function recentInteraction(bdObj: Birthday): Promise<InteractionReplyOptions | string> {
  return {
    embeds: [{
      color: 0xFFFFFF,
      title: '這3個月內生日的人：',
...
async function listInteraction(bdObj: Birthday): Promise<InteractionReplyOptions | string> {
...
async function addInteraction(bdObj: Birthday, options: ChatInputCommandInteraction['options']): Promise<InteractionReplyOptions | string> {
...
async function deleteInteraction(bdObj: Birthday, options: ChatInputCommandInteraction['options']): Promise<InteractionReplyOptions | string> {
  const user = options.getUser('user', true)
  const userid = user.id
...
async function cardInteraction(bdObj: Birthday, options: ChatInputCommandInteraction['options'], member: GuildMember): Promise<InteractionReplyOptions | string> {
...
export default {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (action) {
      const reply = await action(bdObj, interaction.options, interaction.member as GuildMember)
